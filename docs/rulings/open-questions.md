...
- **Status:** RESOLVED by `RULING-2026-08-11-006`.
...
- **Status:** RESOLVED by `RULING-2026-08-11-007`.
...
- **Status:** RESOLVED AND SEQUENCED by `RULING-2026-08-11-008`.
...
- **Status:** OPEN - tracked by GitHub issue `#9`; Questionnaire Part 4 is pending from
  Kamal, AN, and OM.
...
- **Status:** PARTIALLY RESOLVED by `RULING-2026-08-11-004`; exact address/location
  columns await the promised formal Data Model update and ERD review. Tracked by GitHub
  issue `#10`.
...
- **Status:** RESOLVED by `RULING-2026-08-11-005`; company email is the permanent unique
  login identifier. Authentication-provider selection is a technical ADR before auth
  implementation, not a missing user identity field. Tracked by GitHub issue `#11`.
...
- **Resolution:** The immutable login identifier is ruled. Provider selection and
  external-subject representation are handled in a technical ADR without storing
  secrets in `app_user`.
- **Blocks:** Authentication implementation only; it does not block the ruled
  `login_email` field or current conceptual relationship review.

## Supplied-Data Questions

### DATA-001: Client external identifier

- **Status:** STRUCTURAL RULE RESOLVED by `RULING-2026-08-11-004`; legacy blank and
  duplicate values remain held for reconciliation before import.
...
- **Status:** DEFERRED BY OWNER to the legacy import phase.
...
- **Status:** RESOLVED by `RULING-2026-08-11-009`; reference sample only.
